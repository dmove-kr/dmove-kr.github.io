...
### 1-2 Multi-page dashboards (다중 페이지 대시보드)
...
### 1-3 Scheduled PDF reports (PDF 보고서 예약 기능)
...
### 2-3 **New set of functions (새로운 기능)**  
...
### 2-5 Easy-to-manage templates (관리하기 쉬운 템플릿)
...
### 2-7 Scalability improvements (확장성 향상)
...
### 2-8 Universal global scripts (범용 글로벌 스크립트)
...
- Zabbix 5.4에서는  value map을 지원하도록 확장하여 범위에 따라서 0에서 127 사이의 상태를 받으면 서비스를 ' **Down** '으로 간주하고 다른 값 ( ' **Up** '또는 그 반대)으로 간주하도록 정의 할 수 있습니다.
...
### 2-10 Value maps for templates and hosts (템플릿과 호스트에 대한 Value map)
...
### 2-11 Usability improvements (사용성 개선)
...
### 2-13 More improvements (더 많은 개선)
...
### 2-14 New integrations and tempaltes (새로운 통합 및 템플릿)
...
### 3-2 Integration with Grafana (Grafana와의 통합)
